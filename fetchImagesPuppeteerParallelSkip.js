...
// Skips products with duplicate names and those with existing images
// Usage: npm install puppeteer
//        node fetchImagesPuppeteerParallelSkip.js

import fs from 'fs';
import puppeteer from 'puppeteer';

const INPUT_FILE = 'public/products_with_images.json'; // Use the file with images if restarting
const OUTPUT_FILE = 'public/products_with_images.json';
const BATCH_SIZE = 5; // Number of parallel browsers
...
async function processBatch(batch, startIdx) {
  const results = await Promise.all(
    batch.map(async (product) => {
      if (seenNames.has(product['']) || product.image) {
