...
// Ensures each unique product name is processed once, skips duplicates and products with images
// Usage: npm install puppeteer
//        node fetchImagesPuppeteerParallelUnique.js

import fs from 'fs';
import puppeteer from 'puppeteer';

const INPUT_FILE = 'public/products_with_images.json';
const OUTPUT_FILE = 'public/products_with_images.json';
const BATCH_SIZE = 10; // Number of parallel browsers
const SAVE_INTERVAL = 20; // Save every 20 products
const DELAY_MS = 500; // Delay between batches (ms)

let products = JSON.parse(fs.readFileSync(INPUT_FILE, 'utf8'));
const total = products.length;

// Track seen names to skip duplicates
const seenNames = new Set();
products.forEach(p => {
  if (p.image) seenNames.add(p['']);
});
...
async function processBatch(batch, startIdx) {
  const results = await Promise.all(
    batch.map(async (product) => {
      if (seenNames.has(product[''])) {
        console.log(`Skipped: ${product['']}`);
        return product;
      }
      product.image = product.image || await getImage(product['__2']);
      seenNames.add(product['']);
